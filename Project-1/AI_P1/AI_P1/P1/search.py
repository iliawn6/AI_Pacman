...
def depthFirstSearch(problem):
    ...
    while not stack.isEmpty():
        ...
        if state in exploredNodes:
            continue
        else:
            exploredNodes.append(state)
            if problem.isGoalState(state):
                return actions
            else:
                successors = problem.getSuccessors(state)
                for successorState, successorAction, successorCost in successors:
                    newact = actions + [successorAction]
                    newNode = (successorState, newact)
                    stack.push(newNode)
    return actions  
    "*** YOUR CODE HERE ----done----***"
        ...
        if (state not in exploredNodes) or (cost < exploredNodes[state]):
            ...
            if problem.isGoalState(state):
                ...
                for successorState, successorAction, successorCost in successors:
                    ...
                    newNode = (successorState, newact, newCost)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not pq.isEmpty():
        ...
        if problem.isGoalState(state):
            ...
            for successorState, successorAction, successorCost in successors:
                newAct = actions + [successorAction]
                newCost = problem.getCostOfActions(newAct)
                newNode = (successorState, newAct, newCost)
                ...
                if not already_explored:
                    pq.push(newNode, newCost + heuristic(successorState, problem))
                    exploredNodes.append((successorState, newCost))
