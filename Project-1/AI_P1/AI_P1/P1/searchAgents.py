...
def euclideanHeuristic(position, problem, info={}):
    ...
    coo1 = position
    coo2 = problem.goal
    return ((coo1[0] - coo2[0]) ** 2 + (coo1[1] - coo2[1]) ** 2) ** 0.5
...
def cornersHeuristic(state, problem):
    ...
    if len(unvisitedCorners) > 0:
        ...
        cPoint = 0
        cPointCost = util.manhattanDistance(currentLocation, unvisitedCorners[0] )
        for j in range(len(unvisitedCorners)):
            cornerLocation = unvisitedCorners[j]
            length = util.manhattanDistance(currentLocation, cornerLocation )
            
            if length < cPointCost:
                cPoint = j
                cPointCost = length


        closestPoint = (cPoint, cPointCost)    


        fPoint = 0
        fPointCost = util.manhattanDistance(currentLocation, unvisitedCorners[0] )
        for j in range(len(unvisitedCorners)):
            cornerLocation = unvisitedCorners[j]
            length = util.manhattanDistance(currentLocation, cornerLocation )
            
            if length > fPointCost:
                fPoint = j
                fPointCost = length


        farthestPoint = (fPoint,fPointCost)    

        cpIndex = closestPoint[0]
        fpIndex = farthestPoint[0]

        currentNode = problem.startingGameState
        closestNode = unvisitedCorners[cpIndex]
        farthestNode = unvisitedCorners[fpIndex]
...
def foodHeuristic(state, problem):
    ...
    if len(foodList) > 0:
        ...
        cPoint = 0
        cPointCost = util.manhattanDistance(position,foodList[0])
        for j in range(len(foodList)):
            cornerLocation = foodList[j]
            length = util.manhattanDistance(position, cornerLocation )
        ...
        fPoint = 0
        fPointCost = util.manhattanDistance(position, foodList[0])
        for j in range(len(foodList)):
            cornerLocation = foodList[j]
            length = util.manhattanDistance(position, cornerLocation )
        ...
        closestFoodNode = foodList[cpIndex]
