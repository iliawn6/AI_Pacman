    ...
    def chooseAction(self, gameState):
        ...
        for position in livingGhostPositionDistributions:
            if distance == 0 or self.distancer.getDistance(pacmanPosition, position.argMax()) < distance:
                closestPos = position.argMax()
                distance = self.distancer.getDistance(pacmanPosition, position.argMax())

        ret = None
        for action in legal:
            if self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), closestPos) <= distance:
                ret = action
                distance = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), closestPos)
        return ret
